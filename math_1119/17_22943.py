#1번 조건 [2 3 5 7 11...] 2+3=5  2+5=7 2+7=9 같이 계산가능해야함
#2번 조건 2*2 2*3 2*5
#2 11  ->[10, 14, 15, 21, 25, 26, 34, 38, 39, 46, 49, 58, 62, 69, 74, 82, 85, 86, 91, 94]
...
def one(x,num_li):
    for s in list(map(int,str(x))): 
        num_li[s] += 1
        if num_li[s] == 2: return False #만약 숫자 2개 쓰이면 바로 break
        return True

# K 숫자 1개씩만 최대 98765이므로 
...
nCr = itertools.combinations(prime, 2)
sum_li=[x+y for x,y in nCr]

nPr =nPr = itertools.permutations(prime, 2)
mul_li =[ x*y for x,y in nPr]
ans_li=[]
tot_li = list(set(sum_li)& set(mul_li))
...
for x in tot_li:
    ...
    if one(x,num_li) and x %m in tot_li : ans_li.append(x)
